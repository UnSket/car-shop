...
const server = setupServer(
    ...
    rest.get('https://auto1-mock-server.herokuapp.com/api/manufacturers', (req, res, ctx) => {
        return res(ctx.json({manufacturers: [{name: 'Fiat'}, {name: 'Audi'}]}))
    }),
)
...
test('displays current filter', async () => {
    ...
    render(<Filter currentFilter={currentFilter} onFilterSubmit={mockFunction} />, {wrapper: Wrapper});

    await waitFor(() => screen.getByTestId('filter-select-Color'))

    expect(screen.getByTestId('filter-select-Color')).toHaveTextContent('red');
    expect(screen.getByTestId('filter-select-Manufacturer')).toHaveTextContent('Fiat');
});
...
test('change filter', async () => {
    ...
    await waitFor(() => screen.getByTestId('filter-select-Color'));
    const button = getByRole(screen.getByTestId('filter-select-Color'), 'button');
    fireEvent.mouseDown(button);

    await waitFor(() => screen.getByTestId('filter-select-Color-item-green'));

    expect(screen.getByTestId('filter-select-Color-item-green')).toBeInTheDocument();
